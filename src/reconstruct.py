...
def get_limited_slopes(U_i_j: np.ndarray, U_ip1_j: np.ndarray, U_im1_j: np.ndarray,
    ...
    i_up = np.where(delta_ipoh > 0.)
    i_dn = np.where(delta_ipoh <= 0.)
    j_up = np.where(delta_jpoh > 0.)
    j_dn = np.where(delta_jpoh <= 0.)
